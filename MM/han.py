    ...
    def forward(self, gs, h):
        ...
        for i, new_g in enumerate(gs):
            semantic_embeddings.append(self.gat_layers[i](
                new_g, h).flatten(1))  # flatten because of att heads
        semantic_embeddings = torch.stack(
            semantic_embeddings, dim=1)  # (N, M, D * K)
        out, att_mp = self.semantic_attention(
            semantic_embeddings)  # (N, D * K)
...
class HAN(nn.Module):
    def __init__(self,
        ...
        last_activation = create_activation(
            activation) if encoding else create_activation(None)
